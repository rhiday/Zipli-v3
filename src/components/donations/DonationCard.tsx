...
const DonationCard: React.FC<DonationCardProps> = ({ donation, donorName, pickupTime, className }) => {
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-center gap-1 text-base text-gray-700 mb-1 truncate">
            <span>{numericQuantity}{displayUnit}</span>
            <span className="mx-1">·</span>
            <span className="text-gray-500 truncate">from {displayDonor}</span>
          </div>
          <div className="text-xs text-gray-500 truncate">
            {formatPickupTime(displayTime)}
          </div>
        </div>
      </div>
    </Link>
