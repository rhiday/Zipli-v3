...
export default function AllItemsPage(): React.ReactElement {
    ...
    try {
      ...
      if (filters.type === 'donations') {
        let query = supabase
          .from('donations')
          .select(
            'id, created_at, donor_id, food_item_id, pickup_time, pickup_slots, quantity, status, updated_at, food_item:food_items(name, description, image_url, food_type)'
          )
          .eq('donor_id', user.id);
        if (filters.status)
          query = query.eq(
            'status',
            filters.status as Database['public']['Enums']['donation_status']
          );
        if (filters.foodType)
        ...
        fetchedItems = (data || []).reduce<DonationItem[]>((acc, d) => {
          if (!d || !d.food_item) return acc;
          const foodItem =
            Array.isArray(d.food_item) && d.food_item.length > 0
              ? d.food_item[0]
              : d.food_item;
        ...
        let query = supabase
          .from('requests')
          .select(
            'id, created_at, description, is_recurring, people_count, pickup_date, pickup_end_time, pickup_start_time, status, updated_at, user_id'
          )
          .eq('user_id', user.id);
  ...
  const currentStatusOptions =
    filters.type === 'donations' ? donationStatuses : requestStatuses;
  const quantityLabel =
    filters.type === 'donations' ? 'Min. Quantity' : 'Min. People';
  const dateInputClassName =
    'mt-1 block w-full pl-3 pr-3 py-2 text-base border-primary-25 focus:outline-none focus:ring-1 focus:ring-primary sm:text-sm rounded-md shadow-sm bg-base dark:bg-gray-700 dark:border-gray-600 dark:text-primary dark:placeholder-gray-400';
        ...
        {items.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
            {items.map((item) => {
              ...
              const parseRequestInfo = (desc: string) => {
                ...
                const addressPart =
                  parts
                    ...
                    ?.replace('Address: ', '') || '';
                return { mainPart, address: addressPart };
              };

              const title =
                isDonation && (item as DonationItem).food_item
                  ? (item as DonationItem).food_item.name
                  : !isDonation
                    ? parseRequestInfo((item as RequestItem).description)
                        .mainPart
                    : 'No title';
              ...
              const description =
                isDonation && (item as DonationItem).food_item
                  ...
                  : !isDonation
                    ? `Delivery to: ${parseRequestInfo((item as RequestItem).description).address || 'Address not specified'}`
                    : null;
              ...
              const detailLink = isDonation
                ? `/donate/${item.id}`
                : `/request/${item.id}`;
              const quantityOrPeople = isDonation
                ? `${(item as DonationItem).quantity} ${(item as DonationItem).unit || 'kg'}`
                : `${(item as RequestItem).people_count} people`;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="text-md font-semibold text-primary line-clamp-2 flex items-center">
                      {isDonation ? (
                        ...
                        <HandshakeIcon className="mr-2 h-4 w-4 text-blue-600" />
                      )}
                      {title}
                    </h3>
                    <span
                  ...
                  <p className="mb-1 text-sm text-muted-foreground line-clamp-3 h-12 overflow-hidden">
                    {description || 'No description.'}
                  </p>
                  {isDonation &&
                    (item as DonationItem).food_item &&
                    (item as DonationItem).food_item.food_type && (
                      <p className="text-xs text-muted-foreground mt-1 italic">
                        Type: {(item as DonationItem).food_item.food_type}
                      </p>
                    )}
                  <p className="text-xs text-muted-foreground mt-1">
                    {isDonation ? 'Quantity:' : 'For:'} {quantityOrPeople}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {isDonation &&
                      (item as DonationItem).pickup_time &&
                      formatPickupWindow(item as DonationItem)}
                    {!isDonation && (
                      <>
                        Needed on:{' '}
                        {new Date(
                          (item as RequestItem).pickup_date + 'T00:00:00Z'
                        ...
                        {' from '}
                        {(item as RequestItem).pickup_start_time} to{' '}
                        {(item as RequestItem).pickup_end_time}
                      </>
                    )}
                  </p>
                </Link>
          ...
          <div className="col-span-full py-12 text-center">
            ...
            >
              <path
                ...
                strokeWidth={2}
                d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2zm3-12V3m0 18v-2"
              />
            </svg>
