...
export default function TerminalActivity() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load terminal activity data:', error);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-5">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-xl font-semibold text-gray-900">
            {t('terminalActivity') || 'Terminal Activity'}
          </h1>
          <p className="text-gray-600 text-sm mt-1">
            {t('viewAllOperations') || 'View all terminal operations and activity'}
          </p>
        </div>
      </div>
