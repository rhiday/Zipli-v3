...
const isLoading = (status: StatusType): status is typeof LOADING => status === LOADING;
  ...
  useEffect(() => {
    ...
    const processQRLogin = async () => {
      try {
        ...
        const validateResponse = await fetch(`/api/auth/validate-token?token=${token}`);
        const validationData = await validateResponse.json();
        setDebug(prev => prev + `Validation response: ${validateResponse.status} - ${JSON.stringify(validationData)}\n`);
        ...
        const isValidToken = 
          validateResponse.ok || 
          (validateResponse.status === 403 && validationData.message === 'Token has already been used');
        ...
        if (data.user) {
          setDebug(prev => prev + `Signed in successfully as ${data.user.email}\n`);
          ...
          // As a fallback if the immediate redirect doesn't work, try again after a delay
          setTimeout(() => {
            router.push('/dashboard');
          }, 500);
        }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-cream p-4">
      <div className="w-full max-w-md space-y-6 rounded-lg bg-base p-8 shadow-lg text-center">
        ...
        <div className={`flex items-center justify-center h-16 ${getStatusClass()}`}>
          {isLoading(status) && (
