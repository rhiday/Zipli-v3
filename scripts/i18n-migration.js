...
const getContextualNamespace = (filePath) => {
  ...
  if (relativePath.includes('components/donations/'))
    return 'components.donationCard';
  if (relativePath.includes('components/requests/'))
...
const getAllFiles = (dir, pattern) => {
  ...
  for (const entry of entries) {
    ...
    if (
      stat.isDirectory() &&
      !EXCLUDE_PATTERNS.some((p) => entry.includes(p))
    ) {
...
const updateFileWithTranslations = (
  filePath,
  findings,
  usesTranslations,
  namespace,
  ...
  if (!usesTranslations && !updatedContent.includes('const { t }')) {
    ...
    const functionPattern =
      /((?:export\s+)?(?:default\s+)?function\s+\w+[^{]*{|const\s+\w+[^=]*=>[^{]*{)/;
...
const getTranslationHook = (namespace) => {
  const hooks = {
    ...
    'components.pickupScheduler':
      'const { t } = usePickupSchedulerTranslation()',
    'components.voiceInput': 'const { t } = useVoiceInputTranslation()',
    'components.allergenSelector':
...
async function runMigration() {
  ...
  for (const filePath of allFiles) {
    const { findings, usesTranslations, namespace, content } =
      extractHardcodedStrings(filePath);
