...
const criticalFixes = [
  {
    ...
    fixes: [
      {
        from: /\{t\('pages\.auth\.enter_6digit_code'\)\}/g,
        to: 'placeholder="Enter 6-digit code"',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fixes: [
      {
        from: /\{t\('pages\.donations\.recurring_donation'\)\}/g,
        to: 'title="Recurring Donation"',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fixes: [
      {
        from: /return \{t\('pages\.donations\.schedule'\)\};/g,
        to: "return 'Schedule';",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /\{t\('pages\.donations\._error_creating_food_item'\)\}/g,
        to: "'Error creating food item'",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /import ErrorBoundary from \{t\('common\.componentserrorboundary'\)\};/g,
        to: "import ErrorBoundary from '@/components/ErrorBoundary';",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /<Button type = 'Submit_disabledsaving_classnam'flex-1">/g,
        to: '<Button type="submit" disabled={saving} className="flex-1">',
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /state: t\('common\.default_error'\) \| 'success'/g,
        to: "state: 'default' | 'error' | 'success'",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fixes: [
      {
        from: /type: t\('common\.status\.error'\) \| 'performance'/g,
        to: "type: 'error' | 'performance'",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /if \(entry\.entryType == = 'Firstinput_processingstart' in entry\)/g,
        to: "if (entry.entryType === 'first-input' && 'processingStart' in entry)",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /\{t\('common\.sample_organization'\)\}/g,
        to: "'Sample Organization'",
      },
    ],
  },
  {
    ...
    fixes: [
      {
        from: /placeholder = 'PlaceholderFoodName'/g,
        to: 'placeholder="Enter food name"',
      },
      {
        from: /placeholder = 'PlaceholderQuantity'/g,
        to: 'placeholder="Enter quantity"',
      },
      {
        from: /placeholder = 'SelectAllergens'/g,
        to: 'placeholder="Select allergens"',
      },
      {
        from: /placeholder = 'PlaceholderDescription'/g,
        to: 'placeholder="Enter description"',
      },
      {
        from: /label = 'AllergiesIntolerancesDiets'/g,
        to: 'label="Allergies, Intolerances & Diets"',
      },
      {
        from: /hint = 'PhotosHelpIdentify'/g,
        to: 'hint="Photos help identify your food donation"',
      },
