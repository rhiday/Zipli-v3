...
function runSearch(pattern, description) {
  try {
    const result = execSync(`rg "${pattern}" src --type tsx -c`, { encoding: 'utf8' });
...
function auditComponents() {
  ...
  componentsToAudit.forEach(component => {
    try {
      const result = execSync(`rg "${component}" src --type tsx -c`, { encoding: 'utf8' });
      const matches = result.trim().split('\n').filter(line => line.includes(':'));
...
function auditStylingPatterns() {
  ...
  stylingPatterns.forEach(pattern => {
    ...
    if (pattern.includes('bg-\\[#')) description = 'Arbitrary background colors';
    else if (pattern.includes('text-\\[#')) description = 'Arbitrary text colors';  
    ...
    else if (pattern.includes('rounded-\\[')) description = 'Arbitrary border radius';
    else if (pattern.includes('bg-green')) description = 'Non-design-system green colors';
    else if (pattern.includes('bg-blue')) description = 'Non-design-system blue colors';
    else if (pattern.includes('bg-red')) description = 'Non-design-system red colors';
    
    const violations = runSearch(pattern, description);
    totalViolations += violations;
  });
  
  console.log(`📈 SUMMARY: Found ${totalViolations} total styling violations to fix\n`);
}
