...
files.forEach((filePath) => {
  ...
  content = content.replace(
    /<Button\s+type\s*=\s*'Submit_disabledsaving_classnam'flex-1">/g,
    '<Button type="submit" disabled={saving} className="flex-1">'
  );

  // 9. Fix specific known issues
  content = content.replace(
    /t\('common\.alice'\)s\s+(Kitchen|Restaurant)"/g,
    "'Alice\\'s $1'"
  );
  content = content.replace(
    /\{t\('common\.sample_organization'\)\}/g,
    "'Sample Organization'"
  );
  ...
  content = content.replace(
    /if\s*\(entry\.entryType\s*==\s*=\s*'Firstinput_processingstart'/g,
  ...
  if (content !== originalContent) {
    ...
    totalFixed++;
    console.log(
      `✅ Fixed ${path.relative('/Users/rhiday/Desktop/Zipli/zipli v3', filePath)}`
    );
  }
});
